...
let kDestoryPassword = "kDestoryPassword"

@objc(DestoryPasswordSetupViewController)
public class DestoryPasswordSetupViewController: OWSViewController {
    ...
    lazy private var explanationLabel: LinkingTextView = {
        ...
        case .confirming:
            explanationText = NSLocalizedString("DESTORY_PASSWORD_CREATION_CONFIRMATION_EXPLANATION",
                                                comment: "The explanation of confirmation in the 'destory-password creation' view.")
        }
        ...
        explanationLabel.accessibilityIdentifier = "destoryPasswordCreation.explanationLabel"
    ...
    private let pinTextField: UITextField = {
        ...
        pinTextField.accessibilityIdentifier = "destoryPasswordCreation.pinTextField"
        return pinTextField
    }()
    ...
    private lazy var pinTypeToggle: OWSFlatButton = {
        ...
        pinTypeToggle.accessibilityIdentifier = "destoryPasswordCreation.pinTypeToggle"
    ...
    private let nextButton: OWSFlatButton = {
        ...
        nextButton.accessibilityIdentifier = "destoryPasswordCreation.nextButton"
        return nextButton
    }()
    
    private lazy var clearButton: OWSFlatButton = {
        let text = NSLocalizedString("DESTORY_PASSWORD_CANCEL", comment: "Label for the 'cancel destory-password' button. ")
        ...
        clearButton.accessibilityIdentifier = "destoryPasswordCreation.clearButton"
        return clearButton
    }()
    ...
    private let validationWarningLabel: UILabel = {
        ...
        validationWarningLabel.accessibilityIdentifier = "destoryPasswordCreation.validationWarningLabel"
    ...
    private let recommendationLabel: UILabel = {
        ...
        recommendationLabel.accessibilityIdentifier = "destoryPasswordCreation.recommendationLabel"
    ...
    private let completionHandler: (DestoryPasswordSetupViewController, Error?) -> Void
    ...
    init(
        ...
        completionHandler: @escaping (DestoryPasswordSetupViewController, Error?) -> Void
    ) {
        ...
        if case .confirming = self.initialMode {
            owsFailDebug("destory-password setup flow should never start in the confirming state")
        }
    }

    @objc
    class func creatingRegistrationLock(completionHandler: @escaping (DestoryPasswordSetupViewController, Error?) -> Void) -> DestoryPasswordSetupViewController {
        return .init(mode: .creating, enableRegistrationLock: true, completionHandler: completionHandler)
    }

    @objc
    class func creating(completionHandler: @escaping (DestoryPasswordSetupViewController, Error?) -> Void) -> DestoryPasswordSetupViewController {
        return .init(mode: .creating, completionHandler: completionHandler)
    }

    @objc
    class func changing(completionHandler: @escaping (DestoryPasswordSetupViewController, Error?) -> Void) -> DestoryPasswordSetupViewController {
    ...
    var titleText: String {
        if mode.isConfirming {
            return NSLocalizedString("DESTORY_PASSWORD_CREATION_CONFIRM_TITLE", comment: "Title of the 'destory-password creation' confirmation view.")
        } else if case .recreating = initialMode {
            return NSLocalizedString("DESTORY_PASSWORD_CREATION_RECREATION_TITLE", comment: "Title of the 'destory-password creation' recreation view.")
        } else if initialMode.isChanging {
            return NSLocalizedString("DESTORY_PASSWORD_CREATION_CHANGING_TITLE", comment: "Title of the 'destory-password creation' recreation view.")
        } else {
            return NSLocalizedString("DESTORY_PASSWORD_CREATION_TITLE", comment: "Title of the 'destory-password creation' view.")
        }
    }
    ...
    @objc func clearHandler() {
        UserDefaults.standard.removeObject(forKey: kDestoryPassword)
    ...
    private func tryToContinue() {
        ...
        case .creating, .changing, .recreating:
            let confirmingVC = DestoryPasswordSetupViewController(
    ...
    private func isWeakPin(_ pin: String) -> Bool {
        ...
        for character in normalizedPin {
            guard let current = character.wholeNumberValue else {
                owsFailDebug("numeric destory-password unexpectedly contatined non-numeric characters")
                break
            }
    ...
    private func updateValidationWarnings() {
        ...
        case .tooShort:
            switch pinType {
            case .numeric:
                validationWarningLabel.text = NSLocalizedString("DESTORY_PASSWORD_CREATION_NUMERIC_HINT",
                                                                comment: "Label indicating the user must use at least 4 digits")
            case .alphanumeric:
                validationWarningLabel.text = NSLocalizedString("DESTORY_PASSWORD_CREATION_ALPHANUMERIC_HINT",
                                                                comment: "Label indicating the user must use at least 4 characters")
            }
        case .mismatch:
            validationWarningLabel.text = NSLocalizedString("DESTORY_PASSWORD_CREATION_MISMATCH_ERROR",
                                                            comment: "Label indicating that the attempted DESTORY-PASSWORD does not match the first DESTORY-PASSWORD")
        case .weak:
    ...
    private func updatePinType() {
        ...
        switch pinType {
        case .numeric:
            pinTypeToggle.setTitle(title: NSLocalizedString("DESTORY_PASSWORD_CREATION_CREATE_ALPHANUMERIC",
                                                            comment: "Button asking if the user would like to create an alphanumeric DESTORY-PASSWORD"))
            pinTextField.keyboardType = .asciiCapableNumberPad
            recommendationLabelText = NSLocalizedString("DESTORY_PASSWORD_CREATION_NUMERIC_HINT",
                                                         comment: "Label indicating the user must use at least 4 digits")
        case .alphanumeric:
            pinTypeToggle.setTitle(title: NSLocalizedString("DESTORY_PASSWORD_CREATION_CREATE_NUMERIC",
                                                            comment: "Button asking if the user would like to create an numeric DESTORY-PASSWORD"))
            pinTextField.keyboardType = .default
            recommendationLabelText = NSLocalizedString("DESTORY_PASSWORD_CREATION_ALPHANUMERIC_HINT",
                                                        comment: "Label indicating the user must use at least 4 characters")
        }
        ...
        if mode.isConfirming {
            ...
            recommendationLabel.text = NSLocalizedString("DESTORY_PASSWORD_CREATION_DESTORY_PASSWORD_CONFIRMATION_HINT",
                                                         comment: "Label indication the user must confirm their DESTORY-PASSWORD.")
        } else {
    ...
    private func enable2FAAndContinue(withPin pin: String) {
        ...
        let progressView = AnimatedProgressView(
            loadingText: NSLocalizedString("DESTORY_PASSWORD_CREATION_DESTORY_PASSWORD_PROGRESS",
                                           comment: "Indicates the work we are doing while creating the user's destory-password")
        )
        ...
        UserDefaults.standard.set(pin, forKey: kDestoryPassword)
        self.completionHandler(self, nil)
    }
}
...
extension DestoryPasswordSetupViewController: UITextFieldDelegate {
