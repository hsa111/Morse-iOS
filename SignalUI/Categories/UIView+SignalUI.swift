...
public extension UIButton {
    ...
    @objc
    func set(_ anImage: UIImage?, title: String,
    ...
    private func positionLabelRespectToImage(title: String, position: UIView.ContentMode,
        spacing: CGFloat) {
        let imageSize = self.imageView?.image?.size ?? CGSize.zero //self.imageRect(forContentRect: self.frame)
        ...
        case .bottom:
            titleInsets = UIEdgeInsets(top: (titleSize.height + spacing),
                left: -(imageSize.width), bottom: 0, right: 0)
            imageInsets = UIEdgeInsets(top: -imageSize.height, left: 0, bottom: 0, right: -titleSize.width)
        case .left:
